package dev.galacticmc.hitball.raytracing;
...
public class AABB {
    ...
    private AABB(FallingBlock fallingBlock) {
        this.min = getMin(fallingBlock);
        this.max = getMax(fallingBlock);
    }

    private Vector getMin(FallingBlock fallingBlock) {
        return fallingBlock.getLocation().toVector().add(new Vector(-0.5, 0, -0.5));
    }

    private Vector getMax(FallingBlock fallingBlock) {
        return fallingBlock.getLocation().toVector().add(new Vector(0.5, 1, 0.5));
    }

    // Create an AABB based on a player's hitbox
    public static AABB from(FallingBlock fallingBlock) {
        return new AABB(fallingBlock);
    }
